...
### ANÁLISIS COMPLEMENTARIO. TRIMMING CONSERVADOR
### Objetivo: evaluar sensibilidad del pipeline al preprocesamiento
### de lecturas antes de Salmon, sin reemplazar el análisis principal
############################################################
...
ENV_NAME="analisis_obesidad_trimming"
THREADS=4
IDX="4_indice_transcriptoma_trimming/transcriptoma_index"
...
if ! conda env list | awk '{print $1}' | grep -qx "$ENV_NAME"; then
     conda create -n "$ENV_NAME" -c conda-forge -c bioconda \
          trimmomatic salmon multiqc python=3.10 -y    # Entorno complementario con trimming
fi

conda activate "$ENV_NAME"    # Activa el entorno complementario
...
rm -rf 2_multiqc_trimming 3_lecturas_limpias 4_indice_transcriptoma_trimming 5_cuantificacion_salmon_trimming
mkdir -p 2_multiqc_trimming 3_lecturas_limpias 4_indice_transcriptoma_trimming 5_cuantificacion_salmon_trimming

echo "Inicio del análisis complementario con trimming en: $(pwd)"
echo "Entorno activo: $CONDA_DEFAULT_ENV"
echo "Salmon: $(salmon --version)"

### 3. LIMPIEZA CONSERVADORA DE LECTURAS ####
...
### 4. ÍNDICE DEL TRANSCRIPTOMA CON SALMON ####
...
[[ -f "$IDX/versionInfo.json" ]] || {
     echo "ERROR: Salmon no genero versionInfo.json para el análisis complementario."
     exit 1
}

### 5. CUANTIFICACIÓN COMPLEMENTARIA POR MUESTRA ####
...
for r1_limpio in 3_lecturas_limpias/*_R1_limpio.fastq.gz; do
     ...
     env -u LANGUAGE LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 LC_CTYPE=en_US.UTF-8 salmon quant \
          ...
          -p "$THREADS" \
          --validateMappings \
          -o "5_cuantificacion_salmon_trimming/${base}"    # Genera quant.sf complementario

     [[ -f "5_cuantificacion_salmon_trimming/${base}/quant.sf" ]] || {
          echo "ERROR: No se genero quant.sf complementario para $base."
          exit 1
     }
done
...
multiqc 5_cuantificacion_salmon_trimming -o 2_multiqc_trimming --force    # Reporte separado del flujo principal

echo "Análisis complementario con trimming terminado sin errores."
echo "Archivos quant.sf complementarios generados:"
find 5_cuantificacion_salmon_trimming -name quant.sf | sort
echo "Estos resultados no reemplazan el análisis principal sin trimming."
